...
// Load saved AI summary
$savedAiReport = loadSavedAiReport($pdo, $auditId);
$aiSummaryHtml = '';
if (!empty($savedAiReport['content'])) {
	$aiSummaryHtml = "<div class='report-card' style='page-break-before: always;'>";
	$aiSummaryHtml .= "<h3>AI Executive Summary</h3>";
	$aiSummaryHtml .= markdownToHtml($savedAiReport['content']);
	$aiSummaryHtml .= "<p class='text-muted text-end'><small>Generated by: " . htmlspecialchars($savedAiReport['model'] ?? 'AI') . " &mdash; " . (!empty($savedAiReport['created_at']) ? date('d M Y, H:i', strtotime($savedAiReport['created_at'])) : '') . "</small></p>";
	$aiSummaryHtml .= "</div>";
}
