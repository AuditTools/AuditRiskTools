...
try {
	switch ($action) {
		...
		case 'update_remediation':
			...
			$deadline = $_POST['remediation_deadline'] ?? null;
			...
			$stmt = $pdo->prepare("UPDATE findings SET remediation_status = ?, remediation_deadline = ?, updated_at = NOW() WHERE id = ?");
			$stmt->execute([$status, $deadline ?: null, $findingId]);
		...
		case 'delete':
			...
			updateAuditMetrics($pdo, (int)$finding['audit_id'], true);
